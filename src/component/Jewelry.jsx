...
const JewelryList = () => {
  ...
  const handleWhatsAppClick = (item) => {
    const price = parseFloat(item.addJewelryParagraph) || 0;
    const discount = (price * item.promotionPercentage) / 100;
    const finalPrice = price - discount;

    const message = `Hello! I'm interested in: ${item.addJewelryTitle}\nPrice: Rs ${finalPrice.toFixed(0)}`;
  ...
  return (
    <div className="max-w-7xl mx-auto px-5 py-16">
      ...
      {jewelry.length === 0 ? (
        <p className="text-center text-gray-500 text-lg">No jewelry available</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
          {jewelry.map((item) => {
            const originalPrice = parseFloat(item.addJewelryParagraph) || 0;
            const discount = (originalPrice * item.promotionPercentage) / 100;
