...
const StageList = () => {
  ...
  const handleWhatsAppClick = (stage) => {
    // Parse price from paragraph
    const price = parseFloat(stage.addStageParagraph) || 0;
    const discount = (price * stage.promotionPercentage) / 100;
    const finalPrice = price - discount;
    
    const message = `Hello! I'm interested in: ${stage.addStageTitle}\nPrice: Rs ${finalPrice.toFixed(0)}`;
  ...
  return (
    <div className="max-w-7xl mx-auto px-5 py-16">
      ...
      {stages.length === 0 ? (
        <p className="text-center text-gray-500 text-lg">No stages available</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
          {stages.map((stage) => {
            // Calculate prices from paragraph
            const originalPrice = parseFloat(stage.addStageParagraph) || 0;
            const discount = (originalPrice * stage.promotionPercentage) / 100;
            ...
            return (
              ...
              >
                ...
                <div className="p-4 bg-rose-50">
                  ...
                  {/* WhatsApp Button */}
                  <button
                    onClick={() => handleWhatsAppClick(stage)}
