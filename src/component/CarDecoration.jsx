...
const CarList = () => {
  ...
  const handleWhatsAppClick = (car) => {
    ...
    const price = parseFloat(car.addCarParagraph) || 0;
    const discount = (price * car.promotionPercentage) / 100;
    const finalPrice = price - discount;
    
    const message = `Hello! I'm interested in: ${car.addCarTitle}\nPrice: Rs ${finalPrice.toFixed(0)}`;
  ...
  return (
    <div className="max-w-7xl mx-auto px-5 py-16">
      ...
      {cars.length === 0 ? (
        <p className="text-center text-gray-500 text-lg">No cars available</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
          {cars.map((car) => {
            // Calculate prices from paragraph
            const originalPrice = parseFloat(car.addCarParagraph) || 0;
            const discount = (originalPrice * car.promotionPercentage) / 100;
