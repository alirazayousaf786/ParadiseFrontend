...
const MesairiList = () => {
  const [mesairis, setMesairis] = useState([]);

  useEffect(() => {
    const fetchMesairis = async () => {
      try {
        const res = await axios.get("http://localhost:5000/api/addmesairi");
        setMesairis(res.data);
      } catch (err) {
        console.error("Error fetching mesairis:", err);
      }
    };
    fetchMesairis();
  }, []);
  ...
  const handleWhatsAppClick = (mesairi) => {
    // Parse price from paragraph
    const price = parseFloat(mesairi.addMesairiParagraph) || 0;
    const discount = (price * mesairi.promotionPercentage) / 100;
    const finalPrice = price - discount;
    
    const message = `Hello! I'm interested in: ${mesairi.addMesairiTitle}\nPrice: Rs ${finalPrice.toFixed(0)}`;
  ...
  return (
    <div className="max-w-7xl mx-auto px-5 py-16">
      ...
      {mesairis.length === 0 ? (
        <p className="text-center text-gray-500 text-lg">No mesairis available</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
          {mesairis.map((mesairi) => {
            // Calculate prices from paragraph
            const originalPrice = parseFloat(mesairi.addMesairiParagraph) || 0;
            const discount = (originalPrice * mesairi.promotionPercentage) / 100;
            ...
            return (
              ...
              >
                ...
                <div className="w-full h-56 sm:h-64 lg:h-60 overflow-hidden relative">
                  <img
                    src={`http://localhost:5000${mesairi.imageMesairiURL}`}
                    alt={mesairi.addMesairiTitle}
                    className="w-full h-full object-cover transform hover:scale-105 transition-transform duration-300"
                  />
                  {/* Promotion Badge */}
                  {mesairi.promotionPercentage > 0 && (
                    <span className="absolute top-2 right-2 bg-red-500 text-white font-bold text-sm px-3 py-1 rounded-full shadow-lg">
                      {mesairi.promotionPercentage}% OFF
                    </span>
                  )}
                </div>
                ...
                <div className="p-4 bg-rose-50">
                  ...
                  {/* WhatsApp Button */}
                  <button
                    onClick={() => handleWhatsAppClick(mesairi)}
