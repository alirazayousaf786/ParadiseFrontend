...
const BirthdayList = () => {
  ...
  const handleWhatsAppClick = (birthday) => {
    // Parse price from paragraph
    const price = parseFloat(birthday.addBirthdayParagraph) || 0;
    const discount = (price * birthday.promotionPercentage) / 100;
    const finalPrice = price - discount;
    
    const message = `Hello! I'm interested in: ${birthday.addBirthdayTitle}\nPrice: Rs ${finalPrice.toFixed(0)}`;
  ...
  return (
    <div className="max-w-7xl mx-auto px-5 py-16">
      ...
      {birthdays.length === 0 ? (
        <p className="text-center text-gray-500 text-lg">No birthdays available</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
          {birthdays.map((birthday) => {
            // Calculate prices from paragraph
            const originalPrice = parseFloat(birthday.addBirthdayParagraph) || 0;
            const discount = (originalPrice * birthday.promotionPercentage) / 100;
            ...
            return (
              ...
              >
                ...
                <div className="p-4 bg-rose-50">
                  ...
                  {/* WhatsApp Button */}
                  <button
                    onClick={() => handleWhatsAppClick(birthday)}
