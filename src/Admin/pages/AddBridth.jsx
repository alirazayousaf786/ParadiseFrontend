...
import AddBirthdayPopUp from "../component/AddBirthdayPopUp.jsx";
...
export default function AddBirthday() {
  ...
  useEffect(() => {
    const fetchBirthday = async () => {
      try {
        const res = await axios.get("http://localhost:5000/api/addbirthday");
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/api/addbirthday/${id}`);
  ...
  return (
    <section className="p-4 md:p-8 space-y-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-rose-100 rounded-xl p-6 shadow hover:shadow-lg transition">
          ...
          <p className="text-rose-900 leading-relaxed">
            You can add new Birthday, edit existing ones, or delete them.
            Click the <span className="font-semibold">Add Birthday</span> button
            to open the popup.
          </p>
        </div>
        ...
        <div className="bg-rose-100 rounded-xl p-6 flex flex-col items-center justify-center shadow hover:shadow-lg transition">
          <img src={image} alt="birthday" className="w-24 h-24 mb-4" />
      ...
      <AddBirthdayPopUp
        ...
        onSuccess={(updatedBirthday) => {
          if (editBirthday) {
            setBirthdays(
              birthdays.map((b) =>
                b._id === updatedBirthday._id ? updatedBirthday : b
              )
            );
          } else {
            setBirthdays([...birthdays, updatedBirthday]);
          }
        }}
      />
      ...
      <div className="bg-white rounded-xl shadow p-4 md:p-6">
        <h2 className="text-xl font-bold mb-4">All Birthday Data</h2>
            ...
            <tbody>
              {birthdays.length > 0 ? (
                birthdays.map((birthday) => (
                  <tr key={birthday._id} className="hover:bg-gray-50">
                    <td className="p-3 border font-medium">
                      {birthday.addBirthdayTitle}
                    </td>

                    <td className="p-3 border text-sm text-gray-600 max-w-xs truncate">
                      {birthday.addBirthdayParagraph}
                    </td>

                    <td className="p-3 border text-center">
                      <img
                        src={`http://localhost:5000${birthday.imageBirthdayURL}`}
                        alt=""
